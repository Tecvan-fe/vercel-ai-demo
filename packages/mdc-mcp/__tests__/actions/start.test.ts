...
describe('startAction', () => {
  ...
  it('应该创建并连接到MCP服务器', async () => {
    await startAction(mockComponentsDir);

    // 验证创建了服务器
    expect(createServer).toHaveBeenCalledWith(mockComponentsDir);
    ...
    // 验证日志输出
    expect(logger.info).toHaveBeenCalledWith(expect.stringContaining(mockComponentsDir));
    expect(logger.success).toHaveBeenCalledWith(expect.stringContaining('已启动'));
  });
  ...
  it('应该处理错误情况', async () => {
    ...
    // 验证错误处理
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('测试错误'));
